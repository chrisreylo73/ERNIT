...
const AddMenu = ({ setAddMenuVisible, isAddMenuVisible, data, setData }) => {
	...
	const handleCreateButtonPress = async () => {
		...
		const newModule = {
			...
			taskFinished: false,
			currentDate: new Date().toLocaleDateString("en-US").replace(/\//g, "-"),
			randomTileKeys: updateTileKeys(),
			addBack: 0,
			gridData: gridData,
			// daysCompleted: [],
			daysCompleted: ["1-16-2024", "1-17-2024", "1-18-2024", "1-19-2024"],
			dateCreated: new Date().toLocaleDateString("en-US").replace(/\//g, "-"),
			// currentStreak: 0,
			// highestStreak: 0,
			// completionRate: 0,
			// consistencyRate: 0,
